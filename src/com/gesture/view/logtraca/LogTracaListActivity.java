/**************************************************************************
 * LogTracaListActivity.java, WindowsGesture2 Android
 *
...
/**
 * This class will display LogTraca entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a LogTraca list. A click on an item will get 
 * you to LogTracaShowActivity.
 * - On a tablet, the LogTracaShowFragment will be displayed right next to
 * the list.
 */
public class LogTracaListActivity 
		extends HarmonyFragmentActivity
		implements HarmonyListFragment.OnClickCallback,
				HarmonyListFragment.OnLoadCallback {

	/** Associated list fragment. */
	protected LogTracaListFragment listFragment;
	/** Associated detail fragment if any (in case of tablet). */
	protected LogTracaShowFragment detailFragment;
	/** Last selected item position in the list. */
	private int lastSelectedItemPosition = 0;
	/** Last selected item. */
	private LogTraca lastSelectedItem;
	...
	protected void onPostCreate(Bundle savedInstanceState) {
		...
		this.detailFragment = (LogTracaShowFragment) 
						this.getSupportFragmentManager().findFragmentById(
								R.id.fragment_show);

		this.listFragment = (LogTracaListFragment)
						this.getSupportFragmentManager().findFragmentById(
								R.id.fragment_list);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.activity_logtraca_list);
	}
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (this.isDualMode()) {
			...
			final Intent intent = new Intent(this, LogTracaShowActivity.class);
			final LogTraca item = (LogTraca) l.getItemAtPosition(position);
			Bundle extras = new Bundle();
			extras.putParcelable(LogTraca.PARCEL, item);
	 ...
	 */
	private void loadDetailFragment(LogTraca item) {
	...
	public void onListLoaded() {
		if (this.isDualMode()) {
			int newPosition =
				((LogTracaListAdapter) this.listFragment.getListAdapter())
