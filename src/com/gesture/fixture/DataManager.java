/**************************************************************************
 * DataManager.java, WindowsGesture2 Android
 *
...
public class DataManager {
	...
	public DataManager(final Context ctx, final SQLiteDatabase db) {
		...
		this.adapters.put(PRODUIT,
				new ProduitSQLiteAdapter(ctx));
		this.adapters.get(PRODUIT).open(this.db);
		this.adapters.put(MACHINE,
				new MachineSQLiteAdapter(ctx));
		this.adapters.get(MACHINE).open(this.db);
		this.adapters.put(ZONE,
				new ZoneSQLiteAdapter(ctx));
		this.adapters.get(ZONE).open(this.db);
		this.adapters.put(COMMANDE,
				new CommandeSQLiteAdapter(ctx));
		this.adapters.get(COMMANDE).open(this.db);
		this.adapters.put(LOGTRACA,
				new LogTracaSQLiteAdapter(ctx));
		this.adapters.get(LOGTRACA).open(this.db);
		this.adapters.put(CLIENT,
				new ClientSQLiteAdapter(ctx));
		this.adapters.get(CLIENT).open(this.db);
	}
	/**
    ...
    public Object find(final String nameClass, final int id) {
    	...
    	if (nameClass.equals(PRODUIT)) {
        	ret = ((ProduitSQLiteAdapter)
        							   this.adapters.get(nameClass)).query(id);
    	}
    	if (nameClass.equals(MACHINE)) {
        	ret = ((MachineSQLiteAdapter)
        							   this.adapters.get(nameClass)).query(id);
    	}
    	...
    	if (nameClass.equals(COMMANDE)) {
        	ret = ((CommandeSQLiteAdapter)
        							   this.adapters.get(nameClass)).query(id);
    	}
    	if (nameClass.equals(LOGTRACA)) {
        	ret = ((LogTracaSQLiteAdapter)
        							   this.adapters.get(nameClass)).query(id);
    	}
    	if (nameClass.equals(CLIENT)) {
        	ret = ((ClientSQLiteAdapter)
        							   this.adapters.get(nameClass)).query(id);
    	}
    		...
    		if (object instanceof User) {
    			((UserSQLiteAdapter)
    					this.adapters.get(USER))
    						.remove(((User) object).getId_user());
    		}
    		if (object instanceof Produit) {
    			((ProduitSQLiteAdapter)
    					this.adapters.get(PRODUIT))
    						.remove(((Produit) object).getId_produit());
    		}
    		if (object instanceof Machine) {
    			((MachineSQLiteAdapter)
    					this.adapters.get(MACHINE))
    						.remove(((Machine) object).getId_machine());
    		}
    		if (object instanceof Zone) {
    			((ZoneSQLiteAdapter)
    					this.adapters.get(ZONE))
    						.remove(((Zone) object).getId_zone());
    		}
    		if (object instanceof Commande) {
    			((CommandeSQLiteAdapter)
    					this.adapters.get(COMMANDE))
    						.remove(((Commande) object).getId_cmd());
    		}
    		if (object instanceof LogTraca) {
    			((LogTracaSQLiteAdapter)
    					this.adapters.get(LOGTRACA))
    						.remove(((LogTraca) object).getId_log());
    		}
    		if (object instanceof Client) {
    			((ClientSQLiteAdapter)
    					this.adapters.get(CLIENT))
    						.remove(((Client) object).getId_client());
    		}
    	} catch (Exception ex) {
    		this.isSuccessfull = false;
    	}
